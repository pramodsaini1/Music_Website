<%-- 
    Document   : edit_album
    Created on : 27 Jun, 2022, 10:04:55 AM
    Author     : Genius
--%>
    ...
    try{
        ...
        if(rs.next()){
              if(request.getParameter("code")==null){
                  response.sendRedirect("add_album.jsp?code_err=1");
              }
              else{
  ...
  <div class="row">			
        <div class="col-sm-12 w3-card w3-white">
            <%
                ...
                ResultSet rs1 = st1.executeQuery("select * from album where code='"+code+"' AND status=0");
                if(rs1.next()){
           %>
           <div class="col-sm-12"><center><h2><strong><%out.print(rs1.getString("album_name"));%></strong></h2></center></div>
           <div class="row">
                    <div class="col-sm-5">
                        <img src="../album/<%=rs1.getString("code")%>.jpg" class="img-fluid"> 

                    </div>
                     <div class="col-sm-7">
                         <form method="post" action="update_album.jsp?code=<%=rs1.getString("code")%>">
                             <label>Album Name:</label>
                             <input type="text" name="aname" class="form-control" value="<%=rs1.getString("album_name")%>"><br>
                             <label>Album Description:</label>
                             <textarea name="des" class="form-control"><%=rs1.getString("description")%></textarea><br>
                             <input type="submit" value="submit" class="btn btn-primary">
                         </form>
                             <div class="col-sm-12"><center><strong>Change Album Image</strong></center></div>
                             <form method="post" ACTION="ChangeUploadProcess.jsp?album_code=<%=code%>" name="uploadForm" ENCTYPE='multipart/form-data'>
